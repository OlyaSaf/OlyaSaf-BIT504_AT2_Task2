...
public class GameMain extends JPanel implements MouseListener {
	...
	public GameMain() {
		...
		// DONE: This JPanel fires a MouseEvent on MouseClicked so add required event
		// listener to 'this'.
		...
		// Done: Create a new instance of the game "Board" class. HINT check the
	...
	public void paintComponent(Graphics g) {
		...
		if (currentState == GameState.Playing) {
			...
			if (currentPlayer == Player.Cross) {
				...
				statusBar.setText("Cross's turn");

			} else {

				// DONE: use the status bar to display the message "O"'s Turn
				statusBar.setText("Nought's turn");
	 ...
	 * If no winner then isDraw is called to see if deadlock, if not GameState stays
	 * as PLAYING
	 * 
	 */
	public void updateGame(Player thePlayer, int row, int col) {
		...
		if (board.hasWon(thePlayer, row, col)) {
			...
			// DONE: check which player has won and update the currentstate to the
	 ...
	 * and Empty then current player is added to cell content.
